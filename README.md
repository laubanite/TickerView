...
- **自选股**:实时行情 + 换手率,点击切换图表标的
- **图表区**:日K / 30分K / **分时**(三 tab 共用,速查价位后续里程碑叠线)
- **三 tab 侧栏**:盘前(大盘环境/纪律/持仓卡)/ 盘中(**实时核对表**:结论词5档+大盘门控,里程碑3)/ 盘后(**收盘对账**:规则事实+LLM复盘五问+纪律评分,里程碑4)
...
| 6 计划层(产品方案 v0.1.0) | 🚧 里程碑1 解析器 + 里程碑2 作战地图回测已落地(见 alphaprism/planner/ + backtest_map.py) |
