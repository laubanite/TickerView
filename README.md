...
- **图表区**:日K / 30分K / **分时**(三 tab 共用;**速查卡价位自动叠线**:买区/突破点/减仓红线/生命线)
- **三 tab 侧栏**:盘前(**剧本草稿生成→人工确认→写入作战地图**,里程碑2)/ 盘中(**实时核对表**:结论词5档+大盘门控,里程碑3 + **盘中快照**:规则事实+LLM解读+推手机,§5.4)/ 盘后(**收盘对账**:规则事实+LLM复盘五问+纪律评分,里程碑4)
